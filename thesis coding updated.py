...
profit_day = df.groupby("firm")["profit"].sum().sort_values(ascending=False)

plt.figure()
plt.bar(profit_day.index.astype(str), profit_day.values)
plt.xticks(rotation=45, ha="right")
plt.ylabel("Total Profit ($)")
plt.title("Total Profit by Firm (Strategic Firms Only)")
plt.tight_layout()
plt.show()
